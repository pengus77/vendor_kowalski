...
func init() {
	android.RegisterModuleType("kowalski_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = kowalskiExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = kowalskiExpandVariables(ctx, dep_file)
	...
	cmd := kowalskiExpandVariables(ctx, String(g.properties.Cmd))
